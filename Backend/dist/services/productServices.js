...
exports.seedInitialProducts = exports.getAllProduct = void 0;
...
const seedInitialProducts = async () => {
    try {
        ...
        const exictingProducts = await (0, exports.getAllProduct)();
        if (exictingProducts.length === 0) {
            await productModel_1.productModel.insertMany(products);
            return;
        }
        await productModel_1.productModel.updateMany({ category: { $exists: false } }, { $set: { category: "laptop" } });
        const mobileCount = await productModel_1.productModel.countDocuments({ category: "mobile" });
        if (mobileCount === 0) {
            await productModel_1.productModel.insertMany(products.filter((product) => product.category === "mobile"));
        }
    }
    catch (err) {
